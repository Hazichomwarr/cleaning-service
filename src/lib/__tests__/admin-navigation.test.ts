...
test("defines exactly the current admin destinations", () => {
  assert.deepEqual(ADMIN_NAV_ITEMS.map((item) => [item.label, item.href]), [["Dashboard", "/admin"], ["Requests", "/admin/requests"], ["Customers", "/admin/customers"], ["Schedule", "/admin/schedule"], ["Workers", "/admin/workers"]]);
});
