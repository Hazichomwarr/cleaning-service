...
type ReturningCustomerVerificationProps = {
...
export default function ReturningCustomerVerification({ onStateChange, onPropertySelect, onDifferentProperty, onContinueRegular }: ReturningCustomerVerificationProps) {
  ...
  const verify = async () => {
    ...
    setMode("verified");
    onStateChange?.("VERIFIED_CHOOSING_PROPERTY");
  ...
  const selectProperty = (property: VerifiedCustomerPropertyOption) => {
    ...
    <section className="mb-8 rounded-3xl border border-blue-100 bg-blue-50/70 p-5 sm:p-7" aria-labelledby="returning-customer-title">
      {mode === "verified" ? (
        <div>
          ...
          {propertiesLoading ? <p className="mt-2 text-sm leading-6 text-slate-600">Loading your saved properties…</p> : properties?.length ? <div id="returning-property-chooser" className="mt-6"><h3 className="text-lg font-semibold text-slate-950">Choose a saved property</h3><p className="mt-2 text-sm leading-6 text-slate-600">We’ll use this property for your request and keep its saved details read-only.</p><div className="mt-3 grid gap-3 md:grid-cols-2">{properties.map((property) => <article key={property.id} className={`rounded-2xl border bg-white p-4 ${selectedPropertyId === property.id ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200"}`}><h4 className="font-semibold text-slate-950">{property.label || `${property.address.line1}`}</h4><p className="mt-1 text-sm text-slate-600">{property.address.line1}{property.address.line2 ? `, ${property.address.line2}` : ""}<br />{property.address.city}, {property.address.state} {property.address.postalCode}</p><p className="mt-2 text-xs text-slate-500">{propertyTypeLabel(property.propertyType)}{property.bedrooms === null ? "" : ` · ${property.bedrooms} bedroom${property.bedrooms === 1 ? "" : "s"}`}{property.bathrooms === null ? "" : ` · ${property.bathrooms} bathroom${property.bathrooms === "1" ? "" : "s"}`}{property.approximateSquareFeet === null ? "" : ` · ${property.approximateSquareFeet.toLocaleString()} sq ft`}</p><button type="button" onClick={() => { selectProperty(property); onStateChange?.("VERIFIED"); }} className="mt-4 min-h-10 rounded-lg bg-blue-600 px-4 text-sm font-semibold text-white transition hover:bg-blue-700">{selectedPropertyId === property.id ? "Selected" : "Use this property"}</button></article>)}</div><button type="button" onClick={() => { onDifferentProperty?.(); onStateChange?.("VERIFIED"); }} className="mt-4 text-sm font-semibold text-blue-700 underline underline-offset-4">Use a different property</button></div> : <div id="returning-property-chooser" className="mt-4"><p className="text-sm leading-6 text-slate-600">We don’t have a saved property for you yet. You can continue with a new property below.</p><button type="button" onClick={() => { onDifferentProperty?.(); onStateChange?.("VERIFIED"); }} className="mt-4 min-h-10 rounded-lg bg-blue-600 px-4 text-sm font-semibold text-white">Continue with a new property</button></div>}
          <button type="button" onClick={startAsNewCustomer} className="mt-5 text-sm font-semibold text-slate-600 underline underline-offset-4">Start as a new customer</button>
        </div>
      ) : (
        <>
