...
type ScheduleHistoryItem = { id: string; previousScheduledStart: string | null; previousScheduledEnd: string | null; newScheduledStart: string; newScheduledEnd: string | null; reason: string | null; changedAt: string; changedBy: { id: string; name: string; email: string } };
...
export default function RequestScheduleActions({ requestId, status, preferredLabel, confirmedSchedule, scheduleHistory }: { requestId: string; status: CleaningRequestStatus; preferredLabel: string; confirmedSchedule: ScheduleValue; scheduleHistory: ScheduleHistoryItem[] }) {
  ...
  const [appointmentTime, setAppointmentTime] = useState(initialStart.time);
  const [reason, setReason] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();
  const editable = status === "REVIEWING";
  const hasCurrentSchedule = Boolean(confirmedSchedule?.start);

  const openEditor = () => {
    if (!editable || pending) return;
    const currentStart = inputParts(confirmedSchedule?.start ?? null);
    setDate(currentStart.date);
    setAppointmentTime(currentStart.time);
    setReason("");
    setError(null);
    setOpen(true);
  };
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await setCleaningRequestConfirmedScheduleAction({ cleaningRequestId: requestId, date, appointmentTime, reason });
      if (result.success) {
        setOpen(false);
        router.refresh();
        return;
  ...
  return <>
    <div className="mt-5"><p className="text-sm text-slate-500">Customer preferred: <span className="font-medium text-slate-700">{preferredLabel}</span></p><div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm text-slate-500">Confirmed appointment</p><p className="mt-1 text-sm font-semibold text-slate-900">{confirmedSchedule?.start ? formatBusinessDateTime(confirmedSchedule.start) : "Not scheduled yet"}</p></div>{editable ? <button type="button" onClick={openEditor} disabled={pending} className="min-h-11 rounded-xl bg-blue-700 px-4 font-semibold text-white hover:bg-blue-800 disabled:cursor-not-allowed disabled:bg-slate-400">{hasCurrentSchedule ? "Change confirmed appointment" : "Set confirmed appointment"}</button> : null}</div></div>
    {open ? <div role="dialog" aria-modal="true" aria-labelledby="confirmed-schedule-heading" className="mt-5 rounded-xl border border-blue-200 bg-blue-50 p-4 sm:p-5"><div className="flex items-start justify-between gap-4"><div><h3 id="confirmed-schedule-heading" className="font-semibold text-slate-950">{hasCurrentSchedule ? "Change confirmed appointment" : "Set confirmed appointment"}</h3><p className="mt-1 text-sm text-slate-600">Times use the ${BUSINESS.name} business timezone: America/New_York.</p></div><button type="button" onClick={closeEditor} disabled={pending} aria-label="Close confirmed appointment dialog" className="text-2xl leading-none text-slate-400 hover:text-slate-700 disabled:opacity-50">×</button></div><form onSubmit={submit} className="mt-4 space-y-4"><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-semibold text-slate-700">Date<input type="date" required value={date} onChange={(event) => setDate(event.target.value)} className="mt-2 block min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 font-normal text-slate-800" /></label><label className="block text-sm font-semibold text-slate-700">Appointment time<input type="time" required value={appointmentTime} onChange={(event) => setAppointmentTime(event.target.value)} className="mt-2 block min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 font-normal text-slate-800" /></label></div><label htmlFor="schedule-change-reason" className="block text-sm font-semibold text-slate-700">Reason{hasCurrentSchedule ? <span className="font-normal text-slate-500"> (required for a change)</span> : <span className="font-normal text-slate-500"> (optional)</span>}<textarea id="schedule-change-reason" value={reason} onChange={(event) => setReason(event.target.value)} maxLength={500} rows={3} placeholder={hasCurrentSchedule ? "Explain why the appointment is changing..." : "Optional context for setting this appointment..."} className="mt-2 block min-h-24 w-full rounded-xl border border-slate-300 bg-white px-3 py-3 font-normal text-slate-800" /></label>{error ? <p role="alert" className="text-sm text-rose-700">{error}</p> : null}<div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={closeEditor} disabled={pending} className="min-h-11 rounded-xl border border-slate-300 bg-white px-5 font-semibold text-slate-700 disabled:opacity-50">Cancel</button><button type="submit" disabled={pending} className="min-h-11 rounded-xl bg-blue-700 px-5 font-semibold text-white hover:bg-blue-800 disabled:cursor-not-allowed disabled:bg-slate-400">{pending ? "Saving appointment…" : "Save confirmed appointment"}</button></div></form></div> : null}
    <div className="mt-6 border-t border-slate-100 pt-5"><h3 className="text-sm font-semibold text-slate-800">Schedule history</h3>{scheduleHistory.length === 0 ? <p className="mt-3 rounded-xl bg-slate-50 px-4 py-4 text-sm text-slate-600">No confirmed appointment changes yet.</p> : <ol className="mt-4 space-y-4">{scheduleHistory.map((item) => <li key={item.id} className="rounded-xl bg-slate-50 px-4 py-4"><p className="font-semibold text-slate-900">{item.previousScheduledStart === null ? "Confirmed appointment set" : "Confirmed appointment changed"}</p><p className="mt-2 text-sm text-slate-700">{item.previousScheduledStart ? formatBusinessDateTime(item.previousScheduledStart) : "Not scheduled"} → {formatBusinessDateTime(item.newScheduledStart)}</p><p className="mt-2 text-xs text-slate-500"><time dateTime={item.changedAt}>{new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit", timeZone: "America/New_York" }).format(new Date(item.changedAt))}</time> · {item.changedBy.name}<span className="block text-slate-400">{item.changedBy.email}</span></p>{item.reason ? <div className="mt-3 border-t border-slate-200 pt-3 text-sm text-slate-700"><p className="font-semibold text-slate-600">Reason</p><p className="mt-1 whitespace-pre-wrap break-words">{item.reason}</p></div> : null}</li>)}</ol>}</div>
  </>;
}
